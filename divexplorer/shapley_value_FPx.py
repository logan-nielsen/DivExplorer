...
def approximateGlobalMeanShapleyValue_AllSubsets(item_score, K=5000, rng=None):
    ...
    This directly replaces the exact enumeration version.

    Parameters
    ----------
    item_score : dict
        Mapping size -> {frozenset: score}.
        Example: item_score[1][frozenset({'A=1'})] = score
    K : int
        Number of Monte Carlo random permutations.
    ...
    # All singletons
    itemsets_l1 = list(item_score[1].keys())  # list of frozensets
    all_items = itemsets_l1[:]                # universe for permutations

    # Shapley accumulator for each singleton
    shap_acc = {i: 0.0 for i in itemsets_l1}

    # For quick score lookup
    # item_score[size][frozenset] = score

    for _ in range(K):
        perm = list(all_items)
        rng.shuffle(perm)

        S = frozenset()  # current coalition

        # track v(S) once
        v_S = item_score[0][frozenset()]

        for p in perm:
            p_item = p           # frozenset({'attribute=value'})
            S_after = S | p_item

            v_after = item_score[len(S_after)][S_after]
            delta = v_after - v_S

            # Only accumulate for singletons (all p are singletons here)
            shap_acc[p_item] += delta
    ...
    return {i: shap_acc[i] / K for i in shap_acc}
...
def approximateShapleyItemset(I, scores_l, attributes, card_map, 
                          num_samples=5000, rng=None):
    """
    Monte-Carlo approximation of global Shapley value of itemset I.
    """
    import numpy as np
    ...
    attr_I = [attr(i) for i in I]
    attr_I_set = set(attr_I)

    # Bs = attributes unrelated to I
    Bs = set(attributes) - attr_I_set

    # All J in I_Bs (supersets disjoint from I)
    I_Bs = []
    for k in scores_l:
        for J in scores_l[k]:
            if all(attr(j) in Bs for j in J):
                I_Bs.append(J)
    ...
    sample_idx = rng.randint(0, len(I_Bs), size=num_samples)
    ...
    for idx in sample_idx:
        ...
        if len(JI) not in scores_l or JI not in scores_l[len(JI)]:
            continue

        v_JI = scores_l[len(JI)][JI]
        v_J  = scores_l[len(J)][J]

        B = [attr(j) for j in J]
        attr_BI = B + attr_I

        prod_mb = np.prod([card_map[a] for a in attr_BI])

        w = weight_factor(len(B), len(attributes), len(I), prod_mb)

        u_I += (v_JI - v_J) * w

    return u_I / num_samples
